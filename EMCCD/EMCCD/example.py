...
FLAG, message = camera.SetShutter(typ=1, mode='fully auto', closingtime=0, openingtime=0)
...
FLAG, message = camera.SetEMCCDGain(gain=4)


FLAG, message = camera.SetExposureTime(time=0.001)
...
FLAG, message = camera.SetVSSpeed(index=0)
