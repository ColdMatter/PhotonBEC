...
"""
lq.setPower(laser_set_power)
current_power = lq.getPower()
power_tolerance=0.02
while abs(current_power-laser_set_power) > laser_set_power*power_tolerance:
	#FIXME: can be fooled by correct but unstable laser power 
	print "Waiting for laser power to settle. Set "+str(laser_set_power)+" mW; current "+str(current_power) +" mW"
	time.sleep(2)
"""
current_power = lq.getPower()

title_comment = "Ringdown. Laser power "+str(current_power)+" mW; ND filter factor " + str(nd_filter)
...
for i in range(Nruns-1):
	...
	#
	#SAVE DATA
	parameters = {"laser power":current_power,\
	...
	if background_flag:	
		data_dict.update({"Trigger_bkg":bkg_ch2_data_mean, "PDdata_bkg":bkg_ch1_data_mean})
