#ipython --pylab
#execfile("repeated_images.py")
import pyflycapture
from pbec_analysis import *
from scipy.optimize import leastsq
from scipy.misc import imsave

def grabImage(ts = None,name_extra="",numberOfColours=3):
	"""
	...
	#im = 1-reshape(im_flat,(row,col,numberOfColours))  #note: for some reason, we receive the negative image
	im = 1-reshape(im_flat,(row,col,numberOfColours))  #note: for some reason, we receive the negative image sometimes
	if ts!=None:
		extension=".png"
		fname = TimeStampToFileName(ts,file_end=name_extra+extension,make_folder=True)
		imsave(fname,im)
	return im
...
def fit_and_plot(im_raw,ts,x0,y0,dx,dy,px):
	...
	dump = fit_gaussian_to_image(im,maxfev=200)
