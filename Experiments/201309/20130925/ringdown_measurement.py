...
PDgain_dB=float(raw_input("Please type the photodiode gain in dB...\n")) #Waits for human to type a number
laser_power=float(raw_input("Please type the laser power in mW...\n"))
nd_filter=float(raw_input("Please type the ND filters in beam, divide by... \n"))

title_comment = "PD gain "+str(PDgain_dB)+" dB; Laser power "+str(laser_power)+" mW; ND filter factor " + str(nd_filter)
...
for i in range(Nruns-1):
	...
	parameters = {"laser power":laser_power,\
		"Navg scope":Navg,"Nruns":count+1,"PDgain_dB":PDgain_dB,\
	...
	title_string+="\n CH2: trigger; CH1: post-amplified PD output"
	title_string+="\n"+title_comment
	#
	legend()
	xlabel("t / $\mu$s")
