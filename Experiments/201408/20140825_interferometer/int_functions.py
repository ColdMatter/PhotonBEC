...
def visibility_images(timestamp_to_analyze,y_min,y_max,x_min,x_max,analysis_method,guess):
	...
	#load all image files into image array
	i=0
	for file in os.listdir(img_folder):
		img_fn = img_folder+folder_separator+file
		img = imread(img_fn)
	...
	if rmsVis:
		...
		phase_arr = zeros((x_max-x_min,y_max-y_min))
	...
	if fitVis:
		...
		#test guesses
		#y_fit = guess[0]*exp(-x/guess[1])*sin(guess[2]*x-guess[3])+guess[4]
		#close("all")
		#plot(x,y_fit,'r-')		

		test_fitpars = empty(3)
		for j in range(0,y_max-y_min-1):
			for i in range(0,x_max-x_min-1):
				...
				try:
					...
					if j == test_y:
						if i == test_x:
							#return test pixel fit
							test_fitpars = fitpars
				except:
					a_arr[i,j] = 0
					y0_arr[i,j] = 0
	...
	max_width = 20
	max_vis = amax(vis_arr[test_x-max_width:test_x+max_width,test_y-max_width:test_y+max_width])
...
def voltage_image(cameraLabel,coarse_position,base_voltage,voltage_step,num_steps,turns):
	...
	#save coarse position (mm) and number of turns in metadata
	meta.parameters.update({"coarse_position":coarse_position})
	meta.parameters.update({"turns":turns})
	#initialize arrays
	xvs = []
