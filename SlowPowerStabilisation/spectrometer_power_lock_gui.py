...
if gethostname()=="ph-rnyman-01":
	...
	sys.path.append("D:\\\Control\\SpectrometerPowerLock")
	semilogy_scale = True
elif gethostname()=="ph-photonbec2":
	sys.path.append("Y:\\Control\\PythonPackages\\")
	sys.path.append("Y:\\Control\\SpectrometerPowerLock\\")
...
default_set_output_power=569
default_set_output_power=1000
...
class EmbeddedUpdatingGraph(FigureCanvas):
	...
	def compute_initial_figure(self):
		...
		self.axes212.set_ylabel("Output power (au)",fontsize=label_fontsize)
	...
	def update_figure(self):
		...
		time_numbers = [helper_functions.time_number_from_timestamp(r["ts"]) for r in results]
		float_vouts =  [1e3*float(r["Vout"]) for r in results]
		float_cutoffs = [float(r["output_power"]) for r in results]
		...
		self.axes211.set_ylabel("AOM Control voltage (mV)")
		...
		self.axes212.set_ylabel("Output power (au)")
...
class ApplicationWindow(QtGui.QMainWindow):
	...
	def __init__(self,stabiliser):
		...
		self.setWindowTitle("Output Power Stabilisation")
		...
		self.wavelengths_hbox = QtGui.QHBoxLayout()
		self.stop_go_hbox = QtGui.QHBoxLayout()
		
		self.misc_controls_vbox.addLayout(self.set_point_hbox)
		self.misc_controls_vbox.addLayout(self.wavelengths_hbox)
		self.misc_controls_vbox.addLayout(self.stop_go_hbox)

		self.set_output_power=default_set_output_power
		self.stabiliser.set_point = default_set_output_power
		self.stabiliser.pic.set_point = default_set_output_power
		...
		self.setLCD.display(default_set_output_power)
		self.setText = QtGui.QLineEdit() #JM: such a bad name
		self.currentCutoffText=QtGui.QLCDNumber()
		self.set_point_hbox.addWidget(QtGui.QLabel("Output Power (au)\nset point: "))
		...
		self.set_point_hbox.addWidget(QtGui.QLabel("Current output\n"+"power (au)"))
		self.set_point_hbox.addWidget(self.currentCutoffText)
		
		self.wavelengths_hbox.addWidget(QtGui.QLabel("Set Wavelengths"))
		self.wavelength_labels_vbox = QtGui.QVBoxLayout()
		self.wavelength_text_vbox = QtGui.QVBoxLayout()
		self.wavelength_display_vbox = QtGui.QVBoxLayout() 
		self.misc_controls_vbox = QtGui.QVBoxLayout() 
		self.power_controls_vbox = QtGui.QVBoxLayout() 
		self.metric_menu_vbox = QtGui.QVBoxLayout() 
		self.wavelengths_hbox.addLayout(self.wavelength_labels_vbox)
		self.wavelengths_hbox.addLayout(self.wavelength_text_vbox)
		self.wavelengths_hbox.addLayout(self.wavelength_display_vbox)
		self.wavelengths_hbox.addLayout(self.misc_controls_vbox)
		self.misc_controls_vbox.addLayout(self.power_controls_vbox)
		self.misc_controls_vbox.addLayout(self.metric_menu_vbox)
		#
		self.lambda0_LCD = QtGui.QLCDNumber()
		self.lambda0_text = QtGui.QLineEdit()
		self.wavelength_labels_vbox.addWidget(QtGui.QLabel("lam0"))
		self.wavelength_text_vbox.addWidget(self.lambda0_text)
		self.wavelength_display_vbox.addWidget(self.lambda0_LCD)
		#
		self.lambda1_LCD = QtGui.QLCDNumber()
		self.lambda1_text = QtGui.QLineEdit()
		self.wavelength_labels_vbox.addWidget(QtGui.QLabel("lam1"))
		self.wavelength_text_vbox.addWidget(self.lambda1_text)
		self.wavelength_display_vbox.addWidget(self.lambda1_LCD)
		#
		self.lambda2_LCD = QtGui.QLCDNumber()
		self.lambda2_text = QtGui.QLineEdit()
		self.wavelength_labels_vbox.addWidget(QtGui.QLabel("lam2"))
		self.wavelength_text_vbox.addWidget(self.lambda2_text)
		self.wavelength_display_vbox.addWidget(self.lambda2_LCD)
		#
		self.metric_menu = QtGui.QComboBox()
		self.metric_menu.addItems(["BEC_population","BEC_fraction","Total_population"])
		self.metric_menu_vbox.addWidget(QtGui.QLabel("Output Light Metric"))
		self.metric_menu_vbox.addWidget(self.metric_menu)

		self.power_controls_vbox.addWidget(QtGui.QLabel("Fianium output amplitude"))
		self.power_numbers_hbox = QtGui.QHBoxLayout()
		self.power_controls_vbox.addLayout(self.power_numbers_hbox)
		self.out_amp_LCD = QtGui.QLCDNumber()
		self.out_amp_text = QtGui.QLineEdit()
		self.power_numbers_hbox.addWidget(self.out_amp_text)
		self.power_numbers_hbox.addWidget(self.out_amp_LCD)
		...
		#invoke a callback function whenever it gets a new output_power and set_point
		def update_output_power_display():
			self.currentCutoffText.display(self.stabiliser.output_power)
			self.setLCD.display(self.stabiliser.pic.set_point)

		timer = QtCore.QTimer(self)
		QtCore.QObject.connect(timer, QtCore.SIGNAL("timeout()"), update_output_power_display)
