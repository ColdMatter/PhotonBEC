...
//=============================================================================
//=============================================================================
// $Id: LUTPage.cpp,v 1.32 2010-10-12 19:19:13 soowei Exp $
//=============================================================================
...
namespace FlyCapture2
{
    ...
    const char* LUTPage::sk_bankCombo = "comboboxBankLut";
    ...
    {       
        ...
        unsigned int advFeatureInqRegVal = 0;
        error = m_pCamera->ReadRegister(0x40C, &advFeatureInqRegVal);   
        bool isIIDC = (advFeatureInqRegVal & 0x08000000) != 0;
        if (error == PGRERROR_OK && isIIDC)
        {
            // There is a non zero value in the IIDC LUT quadlet offset
            // register, assume that this is valid and return
            m_pEnable->set_label("Enable LUT (IIDC 1.32 mode)");
        }
        else
        {
            // Looks like IIDC is not supported, try PGR
            unsigned int PGR_Value = 0;
            error = m_pCamera->ReadRegister( 0x1A40, &PGR_Value );
            if ( error == PGRERROR_OK )
            {
                m_pEnable->set_label("Enable LUT (PGR mode)");
            }
            else
            {
                m_pEnable->set_label("Enable LUT");
            }
        }
        ...
        const unsigned int inputMax = data.numEntries;
    ...
    void LUTPage::GetWidgets()
    {
        ...
        m_refXml->get_widget( sk_bankCombo, m_pBank );
        ...
        m_refListStoreBanks = Gtk::ListStore::create( m_channelBanks );
        m_pBank->set_model( m_refListStoreBanks );
    }

    void LUTPage::AttachSignals()
    {
        ...
        m_pBank->signal_changed().connect(
            sigc::mem_fun( *this, &LUTPage::OnBankChange ) );
    ...
    void LUTPage::OnReset()
    {
        m_pCurve->reset();
    }

    void LUTPage::OnLoadFromFile()
    {
    ...
    void LUTPage::OnLoadFromCamera()
    {
        ...
        unsigned int bank = 0;
        error = m_pCamera->GetActiveLUTBank( &bank );
        if( error != PGRERROR_OK )
        {
            ShowErrorMessageDialog( "Error querying LUT support", error );
            return;
        ...
        error = m_pCamera->GetLUTChannel( bank, channel, data.numEntries, pEntries );
        if( error != PGRERROR_OK )
        {
            delete [] pEntries;
    ...
    void LUTPage::OnSetToCamera()
    {
        ...
        const int bank = GetActiveBankComboBox();
        ...
        if( channel == -1 )
        {
            ...
            for( int i = 0; i < 3; i++ )
            {
                error = m_pCamera->SetLUTChannel( bank, i, data.numEntries, pEntries );
                if( error != PGRERROR_OK )
                {
                    delete [] pEntries;
        ...
        else
        {
            ...
            error = m_pCamera->SetLUTChannel( bank, channel, data.numEntries, pEntries );
            if( error != PGRERROR_OK )
            {
                delete [] pEntries;
                ShowErrorMessageDialog( "Error setting LUT channel", error );
    ...
    void LUTPage::OnBankChange()
    {
        Gtk::TreeModel::iterator iter = m_pBank->get_active();
        if ( iter == 0 )
        {
            return;
        }	
        ...
        const int bank = GetActiveBankComboBox();
        m_pCamera->SetActiveLUTBank(bank);
    ...
    int LUTPage::GetActiveChannelComboBox()
	{
		// Get the active row
		Gtk::TreeModel::iterator iter = m_pChannel->get_active();
		if ( iter == 0 )
		...
		return row[m_channelColumns.m_colChannel];
	}

    void LUTPage::PopulateChannelComboBox( unsigned int numChannels )
    {
    ...
    int LUTPage::GetActiveBankComboBox()
    {
        // Get the active row
        Gtk::TreeModel::iterator iter = m_pBank->get_active();
        if ( iter == 0 )
        ...
        return row[m_channelBanks.m_colBank];
    }

    void LUTPage::PopulateBankComboBox( unsigned int numBanks )
    {
        m_refListStoreBanks->clear();	
        ...
        for (unsigned int i=0; i < numBanks; i++)
        {
            ...
            Gtk::TreeModel::Row row = *(m_refListStoreBanks->append());
            row[m_channelBanks.m_colBankStr] = bankStr;
            row[m_channelBanks.m_colBank] = i;
        }

        m_pBank->clear();
        m_pBank->pack_start( m_channelBanks.m_colBankStr );

        unsigned int currBank = 0;
        Error error = m_pCamera->GetActiveLUTBank(&currBank);
        if (error != PGRERROR_OK )
        {
            m_pBank->set_active(0);
            return;
        }

        m_pBank->set_active(currBank);
    }
