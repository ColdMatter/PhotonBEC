...
//=============================================================================
//=============================================================================
// $Id: CamControl.cpp,v 1.56 2010/12/02 18:14:47 hirokim Exp $
//=============================================================================
...
namespace FlyCapture2
{
	...
	bool CamControl::Initialize()
	{
        ...
        m_pLadybugSettingsPage = new LadybugSettingsPage( m_pCamera, m_refXml );
        ...
        m_listPages.push_back( m_pLadybugSettingsPage );
        ...
        m_refXml->get_widget( "notebookCamCtl", m_pNotebook );
        m_pNotebook->signal_switch_page().connect(
            sigc::mem_fun( *this, &CamControl::OnSwitchPage ) );
	...
	{				
		...
		// Check if its a Ladybug camera
		char * res1 = 0;
		res1 = strstr(camInfo.modelName, "Ladybug");

		char * res2 = 0;
		res2 = strstr(camInfo.modelName, "Compressor");

		// Hide Ladybug tab if its not an Ladybug camera
		if(res1 == 0 && res2 == 0)
		{
			Gtk::Widget *page;
			page = m_pNotebook->get_nth_page(m_listPages.size()-1);
			page->hide();
		}
	}
	...
	void CamControl::Show(bool isModal)
	{
        if ( m_pCamera == NULL )
        {
            return;
    ...
    void CamControl::SetTitle( const char *pTitle)
    {
        m_pWindow->set_title( pTitle );
    }
    ...
    void CamControl::OnSwitchPage( GtkNotebookPage* /*page*/, guint page_num )
    {
        if ( page_num == 2 )
        {
            m_pFormat7Page->UpdateModeSettings();
        }
    }
}
