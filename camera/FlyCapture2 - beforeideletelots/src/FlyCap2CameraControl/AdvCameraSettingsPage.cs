...
namespace FlyCap2CameraControl
{
    ...
    {        
        ...
        internal void UpdateMemoryChannelButtons()
        {
            if (base.m_camera.ReadRegister(0x614) == 0)
            {
                // Camera is not streaming
                m_memoryChannelsComboBox.Enabled = true;
                m_memoryChannelsRestoreButton.Enabled = true;
                m_memoryChannelsSaveButton.Enabled = true;
                m_memoryChannelsNotes.Text = string.Format("Note: Saving to the default memory channel restores the camera to factory defaults.");
                m_memoryChannelsNotes.ForeColor = Color.Black;
            }
            else
            {
                m_memoryChannelsComboBox.Enabled = false;
                m_memoryChannelsRestoreButton.Enabled = false;
                m_memoryChannelsSaveButton.Enabled = false;
                m_memoryChannelsNotes.Text = string.Format("Camera must be stopped to change memory channel settings.");
                m_memoryChannelsNotes.ForeColor = Color.Red;
            }
        }
    }
}
