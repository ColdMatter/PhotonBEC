...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    PGRGuid guid;
    error = busMgr.GetCameraFromIndex(3, &guid);
    if (error != PGRERROR_OK)
    {
        PrintError( error );
        return -1;
    ...
    for ( int i=0; i < k_numImages; i++ )
    {
		...
		unsigned int dataLen = convertedImage.GetDataSize();
		int row = convertedImage.GetRows();
		int col = convertedImage.GetCols();
		unsigned char* data = convertedImage.GetData();

		char filename[256];
		sprintf(filename, "ExtendedShutterEx-img-%d.ppm", i);
