...
void InformationPane::GetWidgets()
{
    ...
    // Diagnostics
    m_refXml->get_widget("labelSkippedFrames", m_pLblSkippedFrames);
    m_refXml->get_widget("labelLinkRecoveryCount", m_pLblLinkRecoveryCount);
    m_refXml->get_widget("labelTransmitFailures", m_pLblTransmitFailures);
    m_refXml->get_widget("labelTimeSinceInitialization", m_pLblTimeSinceInitialization);
    m_refXml->get_widget("labelTimeSinceLastBusReset", m_pLblTimeSinceLastBusReset);
	m_refXml->get_widget("labelNumPacketResendRequested", m_pLblPacketResendRequested);
	m_refXml->get_widget("labelNumPacketResendReceived", m_pLblPacketResendReceived);
}
...
{  
    ...
    UpdateDiagnostics( infoStruct.diagnostics );
}
...
void InformationPane::UpdateDiagnostics( DiagnosticsStruct diagnostics )
{
    char entry[128];

    if (diagnostics.skippedFrames != -1)
    {
        sprintf( entry, "%d", diagnostics.skippedFrames );
        m_pLblSkippedFrames->set_text( entry );
    }
    else
    {
        m_pLblSkippedFrames->set_text( "N/A" );
    }   

    if (diagnostics.linkRecoveryCount != -1)
    {
        sprintf( entry, "%d", diagnostics.linkRecoveryCount );
        m_pLblLinkRecoveryCount->set_text( entry );
    }
    else
    {
        m_pLblLinkRecoveryCount->set_text( "N/A" );
    }    

    sprintf( entry, "%d", diagnostics.transmitFailures );
    m_pLblTransmitFailures->set_text( entry );

    m_pLblTimeSinceInitialization->set_text( diagnostics.timeSinceInitialization );

    m_pLblTimeSinceLastBusReset->set_text( diagnostics.timeSinceLastBusReset );

	sprintf( entry, "%d", diagnostics.resendRequested );
	m_pLblPacketResendRequested->set_text( entry );

	sprintf( entry, "%d", diagnostics.resendReceived );
	m_pLblPacketResendReceived->set_text( entry );
}
...
void InformationPane::GetPixelFormatStr( PixelFormat pixFmt, char* pPixFmt )
{
    ...
    case PIXEL_FORMAT_422YUV8_JPEG:
        strcpy( pPixFmt, "YUV 422 (JPEG)" ); break;
    default:
