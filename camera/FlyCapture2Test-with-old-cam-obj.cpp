...
static int camera_serial_mapping[MAX_CAM_COUNT] = {0}; //c++ initalizes to zero anyway but make it explicit
...
DLLFUN int SetupFlyCap(unsigned int serialNumber, char* modelName, char* vendorName, char* sensorInfo,
	...
	for(int h = 0; h < nextHandle; h++) {
		printf("testing h=%d map=%d ser=%d\n", h, camera_serial_mapping[h], serialNumber);
		if(camera_serial_mapping[h] != serialNumber)
			continue;
		// Connect to a camera
	...
	camera_serial_mapping[nextHandle] = serialNumber;
