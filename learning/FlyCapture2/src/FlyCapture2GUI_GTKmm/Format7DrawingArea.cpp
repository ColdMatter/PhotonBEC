...
//=============================================================================
//=============================================================================
// $Id: Format7DrawingArea.cpp,v 1.28 2011-01-07 21:37:16 soowei Exp $
//=============================================================================
...
{    
    ...
    bool Format7DrawingArea::on_expose_event( GdkEventExpose* event )
    {
        ...
        float fRatio = pFrame->property_ratio();
        if (!m_isDisabled)
        {
            fRatio = m_maxWidth / (float)m_maxHeight;       
        }
        ...
        if (m_isDisabled)
        {
            DrawDisabledText(refCairo);
        }
        else
        {
            const unsigned int scaledLeft = static_cast<unsigned int>(ToScaled( m_left ));
            const unsigned int scaledTop = static_cast<unsigned int>(ToScaled( m_top ));
            const unsigned int scaledWidth = static_cast<unsigned int>(ToScaled( m_width ));
            const unsigned int scaledHeight = static_cast<unsigned int>(ToScaled( m_height ));     
    ...
    void Format7DrawingArea::DrawDisabledText(Cairo::RefPtr<Cairo::Context> refCairo)
    {
        ...
        sprintf( 
            cursorPosition,
            "Format 7 is not supported by this camera.");
    ...
    void Format7DrawingArea::Disable()
    {
        m_isDisabled = true;
