        ...
        if (SUCCEEDED(hr))
        {
            ...
            if ( m_showCrosshair )
		    {
                if (SUCCEEDED(hr))
                {
                    ...
                    crosshairCenter.x =  ((imageWidth/2)+((FLOAT)(-posX + posXMax/2))) * drawingAreaSize.width/imageWidth;
                    crosshairCenter.y =  ((imageHeight/2)+((FLOAT)(-posY + posYMax/2)))* drawingAreaSize.height/imageHeight;
                    
                    FLOAT crosshairLength = drawingAreaSize.width *CROSSHAIR_LENGTH ; // crosshair length is equal to 2% of drawing area width
                    FLOAT crosshairWidth = crosshairLength * drawingAreaSize.width / imageWidth;
                    FLOAT crosshairHeight = crosshairLength * drawingAreaSize.height / imageHeight;
                    ...
                    FLOAT strokeWidth = 1.0F / m_zoomLevel; 

                    if (strokeWidth < 5.0F)
                    {
                        // set a minimum value for stroke. 
                        strokeWidth = 5.0F;
                    }
	...
	else
	{
        ...
        else
        {
            ...
            for (unsigned int i=0; i < m_zoomLevelTable.size() - 1; i++)
            {
                const float currVal = m_zoomLevelTable[i];
                const float nextVal = m_zoomLevelTable[i+1];
                if (currVal <= m_zoomLevel*100 && m_zoomLevel*100 < nextVal)
                {
					m_zoomLevelIndex = i;
                }
			}
...
void CFlyCap2_MFCView::OnViewDrawImage()
{
	...
	if (m_isDrawingImage == false)
	{
		//hold the last image
		CFlyCap2_MFCDoc* pDoc = GetDocument();
		pDoc->GetConvertedImage().DeepCopy(&m_holdImage);
	}
}
