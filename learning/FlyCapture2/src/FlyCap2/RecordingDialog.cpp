...
void RecordingDialog::SaveImageToFile(Image* image, ImageRecordingPage::ImageSettings* imageSettings)
{
	...
	BOOL isValidPGMPixelFormat =	pixelFormat == PIXEL_FORMAT_MONO8		|| 
	...
	else
	{
		...
		error = image->Convert(PIXEL_FORMAT_BGR, &convertedImage);
		if (error != PGRERROR_OK)
		{
			throw RecordingException(error.GetDescription());
		}
...
void RecordingDialog::UpdateSupportedImageFormats(unsigned int* rows, unsigned int* cols, PixelFormat* pixelFmt)
{
	...
	error = m_pCameraRec->GetCameraInfo(&camInfo);

	if (camInfo.interfaceType == INTERFACE_GIGE)
	{
		GigECamera* gigeCam = static_cast<GigECamera*>(m_pCameraRec);
	...
	{
		Camera* cam = static_cast<Camera*>(m_pCameraRec);
		...
		else
		{
			...
			m_pCameraRec->GetPropertyInfo(&propInfo);
	...
	// build supported output-image formats
	m_imageRecordingPage.BuildSupportedOutputFormats(*pixelFmt);
}

BOOL RecordingDialog::GetPixelFormatFromVideoMode(VideoMode mode, bool stippled, PixelFormat* pixFormat)
{
	if (pixFormat == NULL)
	{
		return FALSE;
...
BOOL RecordingDialog::GetDimensionsFromVideoMode(VideoMode mode, unsigned int* rows, unsigned int* cols)
{
