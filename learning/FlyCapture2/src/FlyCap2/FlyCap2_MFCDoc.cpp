...
CFlyCap2_MFCDoc::DoGrabLoop()
{
	...
	// Setup supported modes for recording dialog
	m_recordingDlg.UpdateSupportedImageFormats(&rowsPrev, &colsPrev, &pixFormatPrev);
    ...
    while( m_continueGrabThread )
    {
		...
		BOOL modeChangeDetected = (rows != rowsPrev) || (cols != colsPrev) || (pixFormat != pixFormatPrev);
		if (m_recordingDlg.IsRecordingActive())
		{
		...
		if (!m_recordingDlg.IsRecordingActive() &&
			modeChangeDetected                  && 
			m_recordingDlg.IsWindowVisible())
		{
			m_recordingDlg.UpdateSupportedImageFormats(&rowsPrev, &colsPrev, &pixFormatPrev);
		}
...
void CFlyCap2_MFCDoc::OnFileSaveAs()
{
	...
	const CString arcsFilter[uiNumFilters] = {
		...
		"Independent JPEG Group 2000(*.jpg2000, *.jpeg2000)|*.jpg2000; *.jpeg2000" , 
	...
	if(error != PGRERROR_OK)
	{
		AfxMessageBox( "Error retrieving camera info\n" );
		return;
	}
	...
	if (camInfo.interfaceType == INTERFACE_GIGE)
	{
		...
		if(error != PGRERROR_OK)
		{
			AfxMessageBox( "Error retrieving GigE image settings\n" );
			return;
		}
	...
	{
		error = ((Camera*)m_pCamera)->GetVideoModeAndFrameRate(&videoMode, &frameRate);
		if(error != PGRERROR_OK)
		{
			AfxMessageBox( "Error retrieving video mode and frame rate\n" );
			return;
		}
		...
		if (videoMode == VIDEOMODE_FORMAT7)
		{
			...
			error = ((Camera*)m_pCamera)->GetFormat7Configuration(&f7ImageSettings, &packetSize, &percentage);
			if(error != PGRERROR_OK)
			{
				AfxMessageBox( "Error retrieving format 7 settings\n" );
				return;
			}
		...
		else
		{
			...
			if(error != PGRERROR_OK)
			{
				AfxMessageBox( "Error retrieving white balance property\n" );
				return;
			}
		...
		(pixelFormat == PIXEL_FORMAT_RAW16))
	{
		...
		{
			tempRaw.Convert(PIXEL_FORMAT_BGR, &tempProcessed);
		}
	}
	...
	for (unsigned int i = 0; i < (uiNumFilters-1); i++)
	{
		...
		// save file
		Error error = tempProcessed.Save(tempFile, (ImageFileFormat)i);

		// if successful, flag it as supported
		if (error == PGRERROR_OK)
		{
			arcsFilterToUse[i] = arcsFilter[i];
		}
		DeleteFile(tempFile);
	}
    ...
    {
		csTemp = arcsFilterToUse[(m_uiFilterIndex + i) % (uiNumFilters - 1)];
		if(csTemp != "")
		{
			csFilters += csTemp;
			csFilters += "|";   
...
bool CFlyCap2_MFCDoc::Start( PGRGuid guid )
{
	...
	if( strcmp(m_cameraInfo.vendorName , "Point Grey Research") == 0 )
	{
...
void CFlyCap2_MFCDoc::CheckCameraPower()
{
	if(IsGrabThreadRunning()&&m_camCtlDlg.IsVisible())
	{
		...
		Error error = m_pCamera->ReadRegister(0x610, &powerReg);
		if(error == PGRERROR_OK)
		{
...
{
	// TODO: Add your command update UI handler code here
	if ( m_pCamera->IsConnected() == true && m_rawPixelFormat != NULL && (m_rawPixelFormat == FlyCapture2::PixelFormat::PIXEL_FORMAT_RAW12 || m_rawPixelFormat == FlyCapture2::PixelFormat::PIXEL_FORMAT_RAW16 ||m_rawPixelFormat == FlyCapture2::PixelFormat::PIXEL_FORMAT_RAW8) )
    {
		pCmdUI->Enable(TRUE);
    }
    else
