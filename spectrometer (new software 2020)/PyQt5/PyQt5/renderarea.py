...
#        self.timer.start(100) # 100 msec
    ...
    def paintEvent(self, event):
        ...
        while (x < globals.pixels):   # 0 through 2047
           self.points.append(QPointF(float(x), float(16384.0 - globals.spectraldata[x])))
           #self.points.append(QPointF(float(x), float(globals.spectraldata[x])))
           x += 1
        painter.scale(self.width()/globals.pixels, self.height()/16384.0)
        painter.drawPolyline(self.points)
        painter.setPen(self.palette().dark().color()) 
        painter.setBrush(Qt.NoBrush)
        painter.drawRect(0, 0, globals.pixels - 1, 16383)
        return
